...
{
    public static readonly UIType uiType = new UIType("MenuPanel");
    ...
    public override void OnEnable()
    {
        ...
        if (!isCreated)
        {
            ...
            settingsBtn = UIMethods.GetInstance().FindObjectInChild<Button>(ActiveObj, "Settings");
            if (settingsBtn != null)
            {
                Debug.Log("找到设置按钮，添加监听");

                // 配置按钮视觉效果（鼠标悬停、键盘/手柄选中、按下）
                ConfigureButtonVisuals(settingsBtn);
    ...
    private void SubscribeInputEvents()
    {
        ...
        Debug.Log("MenuPanel 已订阅键盘/手柄输入事件");
    }
